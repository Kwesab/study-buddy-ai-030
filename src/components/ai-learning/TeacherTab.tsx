...
interface Lesson { title: string; objective: string; estimatedMinutes: number; }
interface CourseOutline { courseTitle: string; description: string; lessons: Lesson[]; }
...
export default function TeacherTab() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const { data } = await supabase.from("uploads").select("id, file_name, created_at").eq("user_id", user.id).eq("status", "analyzed").order("created_at", { ascending: false });
      setUploads(data || []);
      setLoading(false);
    })();
  }, [user]);
  ...
  useEffect(() => {
    if (isTeaching) scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: "smooth" });
  }, [lessonContent, isTeaching]);

  const getSlideText = async (uploadId: string) => {
    const { data } = await supabase.from("generated_content").select("content").eq("upload_id", uploadId).eq("content_type", "summary").single();
    return data?.content ? (typeof data.content === "string" ? data.content : JSON.stringify(data.content)) : "";
  };
  ...
  const startLesson = async (index: number) => {
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/ai-teach`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}`, apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY },
        body: JSON.stringify({ slideText, topic: outline.lessons[index].title, lessonIndex: index, mode: "teach_lesson" }),
      });
      ...
      while (true) {
        ...
        while ((nl = buffer.indexOf("\n")) !== -1) {
          ...
          try {
            ...
            if (content) { accumulated += content; setLessonContent(accumulated); }
          } catch {}
        }
      }
  ...
  const progressPercent = outline ? Math.round((completedLessons.size / outline.lessons.length) * 100) : 0;
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {uploads.map((upload, i) => (
              <motion.div key={upload.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Card className="cursor-pointer hover:border-primary/50 hover:shadow-md transition-all group" onClick={() => generateOutline(upload)}>
                  <CardContent className="p-5 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{new Date(upload.created_at).toLocaleDateString()}</p>
                    </div>
  ...
  if (generatingOutline) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p className="text-sm text-muted-foreground">AI is creating a structured lesson plan</p>
  ...
  if (outline && currentLessonIndex === null) {
    return (
      <div className="space-y-6">
        ...
        <div>
          <h2 className="text-xl font-display font-bold text-foreground">{outline.courseTitle}</h2>
          <p className="text-muted-foreground mt-1">{outline.description}</p>
          <div className="flex items-center gap-3 mt-3">
            <Badge variant="secondary" className="gap-1"><BookOpen className="w-3 h-3" /> {outline.lessons.length} lessons</Badge>
            <Badge variant="secondary" className="gap-1"><Clock className="w-3 h-3" /> {outline.lessons.reduce((a, l) => a + l.estimatedMinutes, 0)} min</Badge>
          </div>
        </div>
          ...
          {outline.lessons.map((lesson, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                <Card className={`cursor-pointer transition-all hover:shadow-md ${isCompleted ? "border-primary/30 bg-primary/5" : "hover:border-primary/50"}`} onClick={() => startLesson(i)}>
                  <CardContent className="p-4 flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-sm font-bold ${isCompleted ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between mb-4 shrink-0">
          ...
          <Badge variant="outline" className="gap-1"><Sparkles className="w-3 h-3" /> Lesson {currentLessonIndex + 1} of {outline.lessons.length}</Badge>
        </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                {isTeaching && (
                  <div className="mt-4 flex items-center gap-2 text-muted-foreground">
                    <Loader2 className="w-4 h-4 animate-spin" /><span className="text-sm">Teaching...</span>
                  </div>
                )}
              </motion.div>
            ) : (
